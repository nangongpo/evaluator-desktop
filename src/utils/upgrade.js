...
export function checkUpdate(windows) {
  ...
  // 更新前，删除本地安装包
  const updaterCacheDirName = `${settings.electronBuilder.productName}-updater`
  const updatePendingPath = path.join(autoUpdater.app.baseCachePath, updaterCacheDirName, 'pending')
  fs.remove(updatePendingPath).then(() => {
    const message = {
      error: '检查更新出错',
      checking: '正在检查更新...',
    ...
    autoUpdater.on('download-progress', function(progressObj) {
      if (upgradeWindow) {
        upgradeWindow.webContents.send('download-progress', parseInt(progressObj.percent))
      }
    })
    // 更新完成，重启应用
